...
## Un-comment and run the next four lines if the Samsung raw data set is NOT 
## already downloaded and unzipped in the working directory.
...
## This assumes the Sansung data file is already unzipped in the working directory.
...
##
## First, merge the columns of the three test data files, and 3 train data files, 
## using the cbind.
test <- cbind(subjecttest, ytest, xtest)
train <- cbind(subjecttrain, ytrain, xtrain)
## Second, using rbind, merge 'test' and 'train' by rows to create one data set called 'mergeddata'.
...
## "tBodyAcc-mean()-X", which would be more readable as "tBodyAccelerometerMean-X". 
...
## summarize command to calculate the mean of each mearsurement in each group, then
## create a new data set called 'mergeddata2'.
...
## the six actitives, for each of the 30 subjects; now the final step is to write the 
## result to a file named 'tidydataset.txt"
write.table(mergeddata2, file = "tidydataset.txt",row.name=FALSE)
##
## The following code should allow a reader to read in this tidy data set and view 
